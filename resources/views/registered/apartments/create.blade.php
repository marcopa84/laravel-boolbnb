...
@section('main')
  <div class="container my-5">
    ...
    <form action="{{route('registered.apartments.store')}}" method="POST" enctype="multipart/form-data">
       ...
       <div class="form-group">
          <label for="bathrooms_number">N° di bagni</label>
          <input id="bathrooms_number" class="form-control" type="number" name="bathrooms_number" value="{{ old('bathrooms_number') }}" min="1" placeholder="N° bagni disponibili">
          @if($errors->has('bathrooms_number'))
             <div class="error alert alert-danger">{{ $errors->first('bathrooms_number') }}</div>
          @endif
       </div>
       <div class="form-group">
          <label for="size">Metri&sup2; appartamento</label>
          <input id="size" class="form-control" type="number" name="size" value="{{ old('size') }}" min="1" placeholder="Grandezza in m&sup2; dell'appartamento">
          @if($errors->has('size'))
       ...
       <div class="form-group">
          <label for="size">Caratteristiche extra</label>
       ...
       <div class="form-group">
          ...
          <input id="price" class="form-control" type="number" name="price" value="{{ old('price') }}" min="1" step="0.1" placeholder="Inserisci il prezzo per notte">
          @if($errors->has('price'))
             <div class="error alert alert-danger">{{ $errors->first('price') }}</div>
          @endif
       </div>
       <div class="form-group">
          <label for="featured_image">Aggiungi immagine di anteprima dell'appartamento</label>
